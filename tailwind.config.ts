...
const config: Config = {
  ...
  theme: {
    extend: {
      ...
      boxShadow: {
        editorial: "0 24px 40px -12px rgba(0, 47, 80, 0.08)",
        float: "0 20px 48px -24px rgba(0, 47, 80, 0.16)"
      },
      backgroundImage: {
        "premium-gradient": "linear-gradient(135deg, rgb(var(--primary)) 0%, rgb(var(--primary-container)) 100%)",
        "paper-glow": "radial-gradient(circle at top right, rgba(162, 202, 247, 0.32), transparent 45%), radial-gradient(circle at bottom left, rgba(207, 229, 255, 0.3), transparent 40%)"
      }
    }
  },
  plugins: []
